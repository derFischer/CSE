...
fuseserver_readlink(fuse_req_t req, fuse_ino_t ino)
{
    ...
    if ((ret = yfs->readlink(ino, dest)) == yfs_client::OK)
    {
        fuse_reply_readlink(req, dest.c_str());
    }
    else
    {
        fuse_reply_err(req, ENOENT);
...
yfs_client::status
fuseserver_symlinkhelper(fuse_ino_t parent, const char *name,
        struct fuse_entry_param *e, const char *dest)
{
    int ret;
    // In yfs, timeouts are always set to 0.0, and generations are always set to 0
    e->attr_timeout = 0.0;
    ...
    yfs_client::inum inum;
    ret = yfs->symlink(parent, name, inum, dest);
    if (ret != yfs_client::OK)
        return ret;
    e->ino = inum;
    ret = getattr(inum, e->attr);
...
void
fuseserver_symlink(fuse_req_t req, const char *dest, fuse_ino_t parent, const char *name)
{
    struct fuse_entry_param e;
    yfs_client::status ret;
    if( (ret = fuseserver_symlinkhelper( parent, name, &e, dest)) == yfs_client::OK ) {
